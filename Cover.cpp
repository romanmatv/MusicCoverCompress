...
bool WriteCoverToQImage(const TagLib::ByteVector& data, QImage& target)
{
    qDebug() << Q_FUNC_INFO << data.data();
    QByteArray arr = QByteArray::fromRawData(data.data(), data.size());
    target = QImage::fromData(arr);

    qDebug() << target << target.isNull();
...
//////////////SAVE TO QIMAGE////////////////////////////////
bool ExtractAPE(TagLib::APE::Tag* tag, QImage& target)
{
    const TagLib::APE::ItemListMap& listMap = tag->itemListMap();
    if (listMap.contains("COVER ART (FRONT)"))
    {
        ...
        {
            const TagLib::ByteVector& pic = item.mid(pos + 1);
            return WriteCoverToQImage(pic, target);
        }
    }
...
bool ExtractID3(TagLib::ID3v2::Tag* tag, QImage& target)
{
    const TagLib::ID3v2::FrameList& frameList = tag->frameList("APIC");
    if (!frameList.isEmpty())
    {
        // Just grab the first image.
        const auto* frame = (TagLib::ID3v2::AttachedPictureFrame*)frameList.front();
        return WriteCoverToQImage(frame->picture(), target);
    }

    return false;
}

bool ExtractASF(TagLib::ASF::File* file, QImage& target)
{
    const TagLib::ASF::AttributeListMap& attrListMap = file->tag()->attributeListMap();
    if (attrListMap.contains("WM/Picture"))
    {
        ...
        if (!attrList.isEmpty())
        {
            ...
            if (wmpic.isValid())
            {
                return WriteCoverToQImage(wmpic.picture(), target);
            }
        }
    }
...
bool ExtractFLAC(TagLib::FLAC::File* file, QImage& target)
{
    const TagLib::List<TagLib::FLAC::Picture*>& picList = file->pictureList();

    qDebug() << "pics:" << picList.size();
    if (!picList.isEmpty())
    {
        // Just grab the first image.
        const TagLib::FLAC::Picture* pic = picList[0];
        return WriteCoverToQImage(pic->data(), target);
    }

    return false;
}

bool ExtractMP4(TagLib::MP4::File* file, QImage& target)
{
    TagLib::MP4::Tag* tag = file->tag();
    const TagLib::MP4::ItemListMap& itemListMap = tag->itemListMap();
    if (itemListMap.contains("covr"))
...
bool CCover::GetEmbedded(const TagLib::FileRef& fr, QImage& target){
    qDebug() << Q_FUNC_INFO << fr.file()->name().wstr();
    bool found = false;
